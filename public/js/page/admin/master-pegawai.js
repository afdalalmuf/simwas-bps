...
$(document).on('keyup', '.select2-search__field', function (e) {
    if (e.key === 'Enter' || e.keyCode === 13) {
        ...
        $.ajax({
            url: `/admin/master-pegawai/getPegawaibyNIP/${searchTerm}`,
            method: "GET",
            cache: false,
            // data: {
            //     _token: token,
            ...
            success: function (response) {
                ...
                $.each(response, function (index, value) {
                    let firstName = value["attributes"]["attribute-first-name"];
                    let lastName = value["attributes"]["attribute-last-name"];
                    let name = value["attributes"]["attribute-first-name"] + " " + value["attributes"]["attribute-last-name"];
                    let nip = value["attributes"]["attribute-nip"];
                    $("#name").append(
                        `<option value="${name}" data-nip="${nip}">${name}</option>`).trigger('change');
                })
