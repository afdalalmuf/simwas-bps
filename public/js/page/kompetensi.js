...
let table = $('#table-kompetensi').DataTable({
    processing: true,
    serverSide: true,
    ajax: {
        url: '/analis-sdm/kelola-kompetensi/data',
        data: function (d) {
            d.unit_kerja = $('#filterUnitKerja').val();
            d.kategori = $('#filterKat').val();
        }
    },
    ...
    buttons: [
        {
            ...
            action: function () {
                window.open('/analis-sdm/kelola-kompetensi/export/excel?unit_kerja=' + $('#filterUnitKerja').val() + '&kategori=' + $('#filterKat').val(), '_blank');
            }
        },
    ],
    columns: [
        ...
        { data: 'nama_pelatihan' },
        { data: 'tgl_mulai' },
        { data: 'sertifikat', orderable: false, searchable: false },
        { data: 'status_text' },
        { data: 'tgl_approve' },
...
$('#filterUnitKerja, #filterKat').on("change", function () {
    table.draw();
});
...
$('#nama_pelatihan').on('change', function () {
    ...
    if (selected === 'lainnya') {
        $('#manual-pelatihan-wrapper').show();
        $('#nama_pelatihan_manual').attr('required', true);

        $('[name="tgl_mulai"]').val('').prop('readonly', false);
        $('[name="tgl_selesai"]').val('').prop('readonly', false);
        $('[name="penyelenggara"]').val('').css({
            pointerEvents: 'auto',
            backgroundColor: '',
        }).trigger('change');
    } else {
        $('#manual-pelatihan-wrapper').hide();
        $('#nama_pelatihan_manual').removeAttr('required').val('');

        $.ajax({
            url: `/pegawai/kompetensi/rencana-diklat/get/${selected}`,
            type: 'GET',
            success: function (data) {
                $('[name="tgl_mulai"]').val(data.start_date).prop('readonly', true);
                $('[name="tgl_selesai"]').val(data.end_date).prop('readonly', true);
                $('[name="penyelenggara"]').val(data.penyelenggara).css({
                    pointerEvents: 'none',
                    backgroundColor: '#e9ecef' // Bootstrap gray
                }).trigger('change');
            },
            error: function () {
                console.warn('Gagal mengambil data pelatihan.');
            }
        });
    }
});
...
$('#pegawai_id').on('change', function () {
    ...
    if (!pegawaiId) {
        $('#nama_pelatihan').html('<option value="">Pilih Pelatihan</option>');
        return;
    }

    $.ajax({
        url: `/analis-sdm/kelola-kompetensi/pegawai/${pegawaiId}/diklat`,
        type: 'GET',
        success: function (response) {
            ...
            $('#nama_pelatihan').html('<option value="">Pilih Pelatihan</option>' + options.join('') + '<option value="lainnya">Lainnya</option>');

        },
        error: function () {
            console.error('Gagal memuat data pelatihan');
        }
    });
});
