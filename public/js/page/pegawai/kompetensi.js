...
const clearError = () => {
    ...
    $("#error-edit-skala").text("");
    $("#error-edit-bentuk").text("");
...
const populateKategori = (bentuk) => {
    $.ajax({
        url: `/pegawai/kompetensi/search-kategori/${bentuk}`,
        type: "GET",
        async: false,
...
$(".bentuk").on("change", function () {
    let bentuk = $(this).val();
    populateKategori(bentuk);
    $(".kat").append('<option value="" disabled selected>Pilih Kategori</option>');
    $(".jenis").append('<option value="" disabled selected>Pilih Jenis</option>').prop("disabled", true);
    $(".teknis").append('<option value="" disabled selected>Pilih Teknis</option>').prop("disabled", true);
});
...
$("#table-kompetensi").on("click", ".edit-btn", function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $("#edit-skala").val(response.data[0].skala).trigger('change');
            $("#edit-bentuk").val(response.data[0].bentuk).trigger('change');
            populateKategori(response.data[0].bentuk);
