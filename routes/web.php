...
use App\Http\Controllers\BpkpController;
use App\Http\Controllers\WordController;
use App\Http\Controllers\SuratController;
use App\Http\Controllers\TugasController;
use App\Http\Controllers\DalnisController;
...
Route::get('/feedback/monitoring', function () {
    return view('feedback.monitoring');
});
...
Route::group(['middleware' => ['auth', 'role:status']], function () {
    ...
    Route::prefix('admin')
        ...
        ->group(function () {
            ...
            Route::post('panduan/upload', [UserGuideController::class, 'upload'])->name('panduan.upload');
            ...
            Route::get('master-pegawai/getPegawaibyNIP/{nip}', [MasterPegawaiController::class, 'getPegawaibyNIP']);
     ...
     * ---------------------------------------------------------------------------
     * */
    Route::prefix('auditee')->name('auditee.')->middleware('role:is_auditee')->group(function () {
        Route::get('dashboard', [AuditeeController::class, 'index'])->name('dashboard');
        Route::get('schedule', [AuditeeController::class, 'schedule'])->name('schedule');
        Route::get('schedule/get', [AuditeeController::class, 'getSchedule'])->name('schedule.get');
        Route::get('schedule/get/{year}', [AuditeeController::class, 'getScheduleByYear'])->name('schedule.getYear');
        Route::get('schedule/detail/{id}', [AuditeeController::class, 'auditDetail'])->name('schedule.detail');
    });
    ...
    Route::prefix('analis-sdm')
        ...
        ->group(function () {
            ...
            Route::get('kelola-kompetensi/export/excel', [AnalisKompetensiController::class, 'exportExcel']);
            Route::get('/kelola-kompetensi/pegawai/{id}/diklat', [AnalisKompetensiController::class, 'getDiklatByPegawai']);
            Route::get('kelola-kompetensi/data', [AnalisKompetensiController::class, 'getRawData']);
            ...
            //Pengecekan SKP
            Route::get('cek-skp', [AnalisCekSKPController::class, 'index'])->name('cek-skp.index');
            Route::get('cek-skp/{id}', [AnalisCekSKPController::class, 'show']);
            Route::post('cek-skp/{id}', [AnalisCekSKPController::class, 'verifikasi']);
            Route::get('skp/chart-data', [DashboardController::class, 'getChartData']);
            Route::post('upload-skp/store', [AnalisCekSKPController::class, 'store']);
            //export penilaian
            Route::get('cek-skp/penilaian/export', [AnalisCekSKPController::class, 'export_skp']);
            //Rencana Diklat\
            Route::post('rencana-diklat/{id}/ajax-update', [RencanaDiklatController::class, 'updateViaAjax'])
                ->name('rencana-diklat.ajax-update');
            Route::post('rencana-diklat/ajax-store', [RencanaDiklatController::class, 'storeViaAjax'])
                ->name('rencana-diklat.ajax-store');
            Route::post('rencana-diklat/import', [RencanaDiklatController::class, 'import']);
            Route::resource('rencana-diklat', RencanaDiklatController::class);
        });
    ...
    Route::prefix('inspektur')
        ...
        ->group(function () {
            ...
            Route::get('skp/chart-data', [DashboardController::class, 'getChartData']);
            Route::get('cek-skp', [InspekturPenilaianKinerjaController::class, 'show_skp']);

            Route::get('skp/chart-irwil1', [DashboardController::class, 'getChartIrwil1']);
            Route::get('skp/chart-irwil2', [DashboardController::class, 'getChartIrwil2']);
            Route::get('skp/chart-irwil3', [DashboardController::class, 'getChartIrwil3']);
            Route::get('skp/chart-buntama', [DashboardController::class, 'getChartBuntama']);
        });
    ...
    Route::prefix('pegawai')->name('pegawai.')->group(function () {
        ...
        Route::get('kompetensi/rencana-diklat/get/{id}', [PegawaiKompetensiController::class, 'getRencanaDiklat']);
        ...
        Route::get('kompetensi/search-kategori/{bentuk}', [PegawaiKompetensiController::class, 'getKategori']);

        //Aktivitas Harian
        Route::get('aktivitas-harian/fetch', [AktivitasHarianController::class, 'fetchData']);
        ...
        //Laporan Bulanan
        Route::get('laporan-bulanan', [PegawaiLaporanKinerjaController::class, 'laporanBulanan'])->name('laporan-bulanan.index');
        Route::get('laporan-bulanan/fetch', [PegawaiLaporanKinerjaController::class, 'getFilteredBulanan'])->name('laporan-bulanan.fetch');
        Route::get('aktivitas/rencana/fetch/{rencanaId}/{year}/{month}', [AktivitasHarianController::class, 'fetchDataperRencana']);
        Route::get('kipappsyncs/getRK/{year}/{month}', [KipappSyncsController::class, 'getRencanaKinerjaKipapp']);
        Route::post('kipappsyncs/store', [KipappSyncsController::class, 'store'])->name('kipappsyncs.store');
        Route::get('kipappsyncs/check', [KipappSyncsController::class, 'check'])->name('kipappsyncs.check');
        Route::post('kipappsyncs/sync', [KipappSyncsController::class, 'sync'])->name('kipappsyncs.sync');
        ...
        //Upload SKP                
        Route::get('upload-skp', [PegawaiSKPController::class, 'index']);
        Route::post('upload-skp/store', [PegawaiSKPController::class, 'store']);
        Route::get('upload-skp/viewSKP/{id}', [PegawaiSKPController::class, 'viewSKP']);
        Route::post('upload-skp/update-penetapan/{id}', [PegawaiSKPController::class, 'update_penetapan']);
        Route::post('upload-skp/update-penilaian/{id}', [PegawaiSKPController::class, 'update_penilaian']);
        Route::get('skp-tim', [PegawaiSKPController::class, 'skp_tim']);

        //SPJ Diklat
        Route::resource('spj-diklat', SpjDiklatController::class);
        Route::get('spj-diklat/edit/{id}', [SpjDiklatController::class, 'edit_spj']);
        Route::post('spj-diklat/kirim/{id}', [SpjDiklatController::class, 'kirim_spj']);
        Route::post('rekening/store', [MasterRekeningController::class, 'store']);
    ...
    Route::prefix('arsiparis')
        ...
        ->group(function () {
            ...
            Route::get('nh/laporan', [ArsiparisNormaHasilController::class, 'index_laporan']);
            Route::get('nh/dokumen', [ArsiparisNormaHasilController::class, 'index_dokumen']);
            Route::get('tim/norma-hasil/viewLaporan/{id}/{jenis}', [TimNormaHasilController::class, 'viewLaporan']);
    ...
    /**
     * ---------------------------------------------------------------------------
     * BPKP - BPKP - BPKP - BPKP - BPKP - BPKP - BPKP
     * ---------------------------------------------------------------------------
     * */
    Route::prefix('bpkp')->name('bpkp.')->middleware('role:is_bpkp')->group(function () {
        Route::get('/', [BpkpController::class, 'index'])->name('dashboard');
        Route::get('dashboard/{id}', [BpkpController::class, 'show']);
    });

    /**
     * ---------------------------------------------------------------------------
     * DALNIS - DALNIS - DALNIS - DALNIS - DALNIS - DALNIS - DALNIS
     * ---------------------------------------------------------------------------
     * */

    Route::prefix('dalnis')->name('dalnis.')->group(function () {
        Route::get('norma-hasil', [DalnisController::class, 'index']);
        Route::get('norma-hasil/{id}', [DalnisController::class, 'show']);
    });

    /**
     * ---------------------------------------------------------------------------
     * VERIFIKATOR - VERIFIKATOR - VERIFIKATOR - VERIFIKATOR - VERIFIKATOR - VERIFIKATOR
     * ---------------------------------------------------------------------------
     * */
    Route::prefix('verifikator-spj')
        ->name('verifikator.')
        ->middleware('role:verifikator-spj')
        ->group(function () {
            Route::get('spj-diklat', [SpjDiklatController::class, 'verifikator_index'])->name('spj-diklat.index');
            Route::get('spj-diklat/show/{id}', [SpjDiklatController::class, 'verifikator_show'])->name('spj-diklat.show');
            Route::post('spj-diklat/save/{id}', [SpjDiklatController::class, 'saveVerification'])->name('spj-diklat.save');
            Route::get('spj-diklat/download-nominatif/{id}', [SpjDiklatController::class, 'generateNominatif'])
                ->name('spj-diklat.download-nominatif');
        });
    ...
    Route::post('objek-pengawasan/update_objek', [ObjekPengawasanController::class, 'update_objek']);
