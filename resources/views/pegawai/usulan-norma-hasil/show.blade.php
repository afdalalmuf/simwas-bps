...
@section('main')
    ...
    {{-- Modal --}}
    <div class="modal fade" id="staticBackdropBatal" data-backdrop="static" data-keyboard="false" tabindex="-1"
        aria-labelledby="staticBackdropBatalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropBatalLabel">Batalkan Nomor Norma Hasil</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form method="post" action="/pegawai/norma-hasil/{{ $usulan->id }}">
                    <div class="modal-body">
                        ...
                        @csrf
                        <input type="hidden" name="id" value="{{ $usulan->id }}">
                        <input type="hidden" name="status_norma_hasil" value="dibatalkan">
                        <div class="form-group">
                            <label for="draft">Alasan Pembatalan Nomor</label>
                            <input placeholder="Berikan Alasan Pembatalan Nomor" required type="text"
                                class="form-control" name="alasan" id="alasan">
                            @error('alasan')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-body">
                            @if ($dalnis == '1')
                                <a class="btn btn-outline-primary" href="/dalnis/norma-hasil">
                                    <i class="fas fa-arrow-left mr-1"></i> Kembali
                                </a>
                            @else
                                <a class="btn btn-outline-primary"
                                    href="{{ route('ketua-tim.usulan-norma-hasil.index') }}">
                                    <i class="fas fa-arrow-left mr-1"></i> Kembali
                                </a>
                            @endif
                            ...
                            <table class="mb-4 table table-striped responsive" id="table-show">
                                @if ($usulan->status_norma_hasil == 'disetujui')
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @else
                                                <span class="badge badge-primary">
                                                    T-{{ $usulan->normaHasilAccepted->nomor_norma_hasil }}/{{ $usulan->normaHasilAccepted->unit_kerja }}/{{ $usulan->normaHasilAccepted->kode_klasifikasi_arsip }}/{{ $usulan->masterLaporan->kode ?? '' }}/{{ date('Y', strtotime($usulan->normaHasilAccepted->tanggal_norma_hasil)) }}
                                                </span>
                                            @endif
                                        </td>
                                    </tr>
                                ...
                                @if (
                                    $usulan->status_norma_hasil == 'disetujui' &&
                                ...
                                <tr>
                                    <th>Status Surat:</th>
                                    @if (
                                        $usulan->status_norma_hasil != 'diperiksa' &&
                                            $usulan->status_norma_hasil != 'ditolak' &&
                                            $usulan->status_norma_hasil != 'dibatalkan')
                                        <td>
                                            @if ($usulan->normaHasilAccepted->status_verifikasi_arsiparis == 'belum unggah')
                                                <span class="badge badge-primary">Menunggu Upload Laporan</span>
                                            @elseif ($usulan->normaHasilAccepted->status_verifikasi_arsiparis == 'diperiksa')
                                                <span class="badge badge-primary">Menunggu Verifikasi
                                                    Arsiparis</span>
                                            @elseif ($usulan->normaHasilAccepted->status_verifikasi_arsiparis == 'disetujui')
                                    ...
                                    {{ $usulan->status_norma_hasil == 'dibatalkan' ? 'badge-dark' : '' }}
                                ...
                                @elseif ($usulan->status_norma_hasil == 'dibatalkan')
                                    <tr>
                                        <th>Alasan Pembatalan Nomor:</th>
                                        <td>{{ $usulan->catatan_norma_hasil }}</td>
                                    </tr>
                                @endif
                            ...
                            @if ($usulan->status_norma_hasil == 'diperiksa')
                                <div class="d-flex align-content-end w-100 justify-content-end" style="gap: 10px;">
                                    ...
                                    <form id="setujui"
                                        action="{{ route('ketua-tim.usulan-norma-hasil.store', $usulan->id) }}"
                                        method="post">
                                        @csrf
                                        @method('POST')
                            ...
                            @elseif ($usulan->status_norma_hasil == 'disetujui')
                                <div class="d-flex align-content-end w-100 justify-content-end" style="gap: 10px;">
                                    <button type="button" class="btn btn-danger" data-toggle="modal"
                                        data-target="#staticBackdropBatal">
                                        <i class="fa-regular fa-circle-xmark mr-1"></i>Batalkan Nomor
                                    </button>
                                </div>
                            @endif
                        </div>
