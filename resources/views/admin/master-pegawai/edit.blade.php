                        ...
                        <div class="card-body">
                            ...
                            <form method="POST" action="/admin/master-pegawai/{{ $user->id }}" class="needs-validation"
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" name="satuan_kerja" required>
                                        <option value="0000"
                                            {{ old('satuan_kerja', $user->satuan_kerja) == '0000' ? 'selected' : '' }}>
                                            BPS PUSAT
                                        </option>
                                        @foreach ($satuan_kerja as $key => $value)
                                            <option value="{{ $value['kode_satuankerja'] }}"
                                                {{ old('satuan_kerja', $user->satuan_kerja) == $value['kode_satuankerja'] ? 'selected' : '' }}>
                                                {{ $value['nama'] }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                @endforeach
                                <hr class="my-1 mb-4 mt-lg-5">
                                @foreach ($master_role as $key => $value)
                                    <div class="form-group">
                                        <label>Sebagai {{ $value->name }} ?</label>
                                        <div class="d-flex align-items-center">
                                            <div class="selectgroup w-50">
                                                <label class="selectgroup-item">
                                                    <input type="radio" name="{{ $value->id }}" value="1"
                                                        class="selectgroup-input"
                                                        {{ in_array($value->id, $pegawai_roles) ? 'checked' : '' }}>
                                                    <span class="selectgroup-button">Ya</span>
                                                </label>
                                                <label class="selectgroup-item">
